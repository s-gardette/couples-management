...
export function SecureSigninForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const emailOrUsername = formDataObj.get('emailOrUsername') as string;
    const password = formDataObj.get('password') as string;

    // Update React state to match actual form values (for autofill scenarios)
    if (emailOrUsername !== formData.emailOrUsername || password !== formData.password) {
      setFormData({
    ...
    try {
      ...
      // Send only the relevant field to avoid unnecessary database queries
      const loginData = isEmailInput 
        ? { email: emailOrUsername, password: password }
        : { username: emailOrUsername, password: password };
      
      await secureAuthClient.login(loginData);
      // Redirect will be handled by the middleware after successful login
